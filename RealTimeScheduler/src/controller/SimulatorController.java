...
public class SimulatorController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        schedulerChoiceBox.setItems(FXCollections.observableArrayList(
            SchedulerFactory.RMS,
                SchedulerFactory.EDF
        ));
    ...
    public void handleStartSimulation()
    {
        ...
        if (simulationRuntimeField.getText().trim().equals("")) {
            scheduler = factory.getScheduler(schedulerName, tasks);
        ...
        if (!scheduler.isSchedulable()) {
            // TODO: Change with a legit report
            ModalWindow.displayError("Scheduler is not schedulable.");
            return;
        }
